...
const { getRealCommandName } = require('./utils');
...
class CommonBin {
  constructor(rawArgv) {
     ...
     */
    if (rawArgv) {
      this.prefixName = rawArgv.prefixName;
    }
  ...
  load(fullPath) {
    ...
    debug(
      '[%s] loaded command `%s` from directory `%s`',
      this.constructor.name,
      names,
      fullPath
    );
  ...
  add(name, target) {
    ...
    if (!(target.prototype instanceof CommonBin)) {
      ...
      assert(
        target.prototype instanceof CommonBin,
      ...
      function* () {
        ...
        if (index !== -1) {
          ...
          this.rawArgv.splice(
            index,
            2,
            `--AUTO_COMPLETIONS=${this.rawArgv.join(',')}`
          );
        }
        yield this[DISPATCH]();
      }.bind(this)
  ...
  errorHandler(err) {
    ...
    console.error(
      chalk.red('⚠️  Command Error, enable `DEBUG=common-bin` for detail')
    );
  ...
  * [DISPATCH]() {
    ...
    const commandRealName = getRealCommandName(commandName, this[COMMANDS]);
    ...
    if (this[COMMANDS].has(commandRealName)) {
      const Command = this[COMMANDS].get(commandRealName);

      const rawArgv = this.rawArgv.slice();
      rawArgv.splice(rawArgv.indexOf(commandName), 1);
      rawArgv.prefixName = this.prefixName
        ? `${this.prefixName} ${commandName}`
        : commandName;
      debug(
        '[%s] dispatch to subcommand `%s` -> `%s` with %j',
        this.constructor.name,
        commandRealName,
        Command.name,
        rawArgv
    ...
    for (const [ name, Command ] of this[COMMANDS].entries()) {
      const commandName = this.prefixName ? `${this.prefixName} ${name}` : name;
      const aliases = [ commandName, ...Command.prototype.aliases ];

      this.yargs.command(aliases, Command.prototype.description || '');
    }
  ...
  get context() {
    ...
    if (this.parserOptions.execArgv) {
      ...
      let {
        ...
        debugOptions,
        execArgvObj,
      } = this.helper.extractExecArgv(argv);
      ...
      if (context.env.NODE_DEBUG_OPTION) {
        console.log(
          'Use $NODE_DEBUG_OPTION: %s',
          context.env.NODE_DEBUG_OPTION
        );
      ...
      if (execArgvObj.expose_debug_as && semver.gte(process.version, '7.0.0')) {
        console.warn(
          chalk.yellow(
            `Node.js runtime is ${
              process.version
            }, and inspector protocol is not support --expose_debug_as`
          )
        );
      }
      ...
      Object.defineProperty(context, 'execArgv', {
        get() {
          ...
          const execArgv = self.helper.unparseArgv(lazyExecArgvObj, {
            excludes: [ 'require' ],
          });
